...
import cv2
import numpy as np
from midas_loss import compute_scale_and_shift, ScaleAndShiftInvariantLoss
import torch
...
def compute_errors(gt, pred):
    ...
    try:
        gtPred = np.log(pred)
        gtPred10 = np.log10(pred)
    except Exception as e:
        pred.clip(0.000001, np.max(pred))
        gtPred = np.log(pred + 1)
        gtPred10 = np.log10(pred + 1)

    rmse_log = (gtLog - gtPred) ** 2
    ...
    err = np.abs(gtPred10 - np.log10(gt))
    log10 = np.mean(err)
...
def test():
    ...
    for i in range(num_test_samples):
        ...
        pred_depth = cv2.imread(pred_depth_path, cv2.IMREAD_GRAYSCALE)
    ...
    if args.dataset == 'kitti':
        ...
        for t_id in range(num_test_samples):
            filename = pred_filenames[t_id]
            gt_depth_path = os.path.join(args.gt_path, filename)
            # print(gt_depth_path)
            depth = cv2.imread(gt_depth_path, cv2.IMREAD_GRAYSCALE)
            # print(depth.shape)
            if depth is None:
                print('Missing: %s ' % gt_depth_path)
                missing_ids.add(t_id)
            ...
            depth = depth.astype(np.float32) / 256.0 # This converts it to 0 - 1
            gt_depths.append(depth)
...
def eval(pred_depths):
    ...
    gt_all = np.zeros((num_samples, *(pred_depths[0].shape)))
    pred_all = np.zeros_like(gt_all)
    masks_all = np.zeros_like(gt_all)
    ...
    for i in range(num_samples):
        ...
        gtShape = pred_depth.shape
        torchPred = torch.from_numpy(pred_depth).reshape((1, *(pred_depth.shape)))
        torchGT = torch.from_numpy(gt_depth).reshape((1, *(gt_depth.shape)))
        torchMask = torch.from_numpy(valid_mask).reshape((1, *(valid_mask.shape)))

        # print(pred_depth.shape, gt_depth.shape, valid_mask.shape)
        scale, shift = compute_scale_and_shift(torchPred, torchGT, torchMask)
        scaledAndShiftedPred = scale.view(-1, 1, 1) * pred_depth + shift.view(-1, 1, 1)
        scaledAndShiftedPred = np.array(scaledAndShiftedPred).reshape(gtShape)
        # scaledAndShiftedPred.clip(1, np.max(scaledAndShiftedPred))
        
        silog[i], log10[i], abs_rel[i], sq_rel[i], rms[i], log_rms[i], d1[i], d2[i], d3[i] = compute_errors(gt_depth[valid_mask], scaledAndShiftedPred[valid_mask])
        ...
        gt_all[i] = gt_depth
        masks_all[i] = valid_mask
    
    scale, shift = compute_scale_and_shift(torch.from_numpy(pred_all), torch.from_numpy(gt_all), torch.from_numpy(masks_all))
    print(f"scale = {scale} | shift = {shift}")
    print(f"scale.shape = {scale.shape} | shift.shape = {shift.shape}")
    lossCalc = ScaleAndShiftInvariantLoss()
    loss = lossCalc.forward(torch.from_numpy(pred_all), torch.from_numpy(gt_all), torch.from_numpy(masks_all))
    print(f"Scale and Shift Invariant Loss = {loss}")
...
Original Metrics
     d1,      d2,      d3,  AbsRel,   SqRel,    RMSE, RMSElog,   SILog,   log10
  0.039,   0.126,   0.313,  13.654,   5.372,   0.348,   1.936, 134.895,   0.614


Scaled and Shifted Metrics
     d1,      d2,      d3,  AbsRel,   SqRel,    RMSE, RMSElog,   SILog,   log10
  0.541,   0.697,   0.769,   3.284,   0.524,   0.130,   2.567, 148.736,   0.911
'''